...
public enum BusinessStatusEnum {
    ...
    public static String findByStatus(String status) {
        if (SmartStringUtil.isEmpty(status)) {
    ...
    public static void checkStartStatus(String status) {
        if (WAITING.getStatus().equals(status)) {
            throw new BusinessException("该单据已提交过申请,正在审批中！");
        } else if (FINISH.getStatus().equals(status)) {
            throw new BusinessException("该单据已完成申请！");
        } else if (INVALID.getStatus().equals(status)) {
            throw new BusinessException("该单据已作废！");
        } else if (TERMINATION.getStatus().equals(status)) {
            throw new BusinessException("该单据已终止！");
        } else if (SmartStringUtil.isEmpty(status)) {
            throw new BusinessException("流程状态为空！");
        }
    }
    ...
    public static void checkCancelStatus(String status) {
        if (CANCEL.getStatus().equals(status)) {
            throw new BusinessException("该单据已撤销！");
        } else if (FINISH.getStatus().equals(status)) {
            throw new BusinessException("该单据已完成申请！");
        } else if (INVALID.getStatus().equals(status)) {
            throw new BusinessException("该单据已作废！");
            ...
            throw new BusinessException("该单据已退回！");
        } else if (SmartStringUtil.isEmpty(status)) {
            throw new BusinessException("流程状态为空！");
        }
    }
